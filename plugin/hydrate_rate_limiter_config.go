...
func (c *HydrateRateLimiterConfig) String() string {
	return fmt.Sprintf("Definitions: %s\nStaticScopeValues: %s\nCost: %d MaxConcurrency: %d", c.Definitions, rate_limiter.FormatStringMap(c.ScopeValues), c.Cost, c.MaxConcurrency)
}
...
func (c *HydrateRateLimiterConfig) initialise(hydrateFunc HydrateFunc) {
	...
	c.ScopeValues[rate_limiter.RateLimiterScopeHydrate] = helpers.GetFunctionName(hydrateFunc)
